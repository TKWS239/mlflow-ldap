...
#custom import for audit logging
import logging
...
# custom logging for user access
@app.after_request
def audit_log(response):
    ...
    model_name = request.args.get('path', 'null')
    run_uuid = request.args.get('run_uuid', 'null')
    experiment_id = request.args.get('experiment_id', 'null')
    # Reconfigure the timezone to utc+8
    utc_plus_8 = pytz.timezone('Asia/Shanghai')
    current_time = datetime.now(pytz.utc).astimezone(utc_plus_8)
    utc_hong_kong_time = current_time.strftime('%Y/%m/%d %H:%M')

    mlflow_log = {
        "datetime": utc_hong_kong_time,
    ...
    # Skip the logging on web page resources, only captures the access of artifacts, login etc including succeed and failure
    if not request.path.startswith(('/ajax-api', '/static-files', '/src')):
        if str(request.path) == '/':
            mlflow_log['request'] = '/login'
        app.logger.info(json.dumps(mlflow_log))
    else:
        return response
    return response
